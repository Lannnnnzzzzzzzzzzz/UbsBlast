...
export const blastApi = {
  sendBulk: (sessionId: string, messages: BulkMessageItem[], options?: BulkMessageOptions) =>
    request<BulkBatchResponse>(`/sessions/${sessionId}/messages/send-bulk`, {
      method: 'POST',
      body: JSON.stringify({ messages, options }),
    }),
  getBatchStatus: (sessionId: string, batchId: string) =>
    request<BatchStatus>(`/sessions/${sessionId}/messages/batch/${batchId}`),
  cancelBatch: (sessionId: string, batchId: string) =>
    request<{ batchId: string; status: string; progress: BatchStatus['progress'] }>(
